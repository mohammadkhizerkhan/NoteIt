...
function Notes() {
  ...
  const filterNotes=()=>{
    ...
    if(filterState.sortBy==="new_to_old"){
      sortedNotes=sortedNotes.sort((note1,note2)=>note2.createdAt-note1.createdAt)
    }
    if(filterState.sortBy==="old_to_new"){
      sortedNotes=sortedNotes.sort((note1,note2)=>note1.createdAt-note2.createdAt)
    }
    if(filterState.sortBy==="A_Z"){
      sortedNotes=sortedNotes.sort((note1,note2)=>note1.title.localeCompare(note2.title))
    }
    if(filterState.sortBy==="Z_A"){
      sortedNotes=sortedNotes.sort((note1,note2)=>note2.title.localeCompare(note1.title))
    }
  ...
  return (
    ...
    {pinnedNotes.length>0 && <h4 className="main-headers">Pinned notes</h4>}
      ...
      {notpinnedNotes.length>0 && <h4 className="main-headers">unpinned notes</h4>}
